  ...
  notebooklmPackage = pkgs.symlinkJoin {
    ...
    paths = [
      (pkgs.writeShellScriptBin "notebooklm-mcp" ''
        ...
        exec ${lib.getExe pkgs.uv} tool run --python ${lib.getExe pkgs.python3} --from notebooklm-mcp-cli==0.9.11 notebooklm-mcp "$@"
      '')
      (pkgs.writeShellScriptBin "nlm" ''
        export UV_PYTHON_DOWNLOADS=never
        exec ${lib.getExe pkgs.uv} tool run --python ${lib.getExe pkgs.python3} --from notebooklm-mcp-cli==0.9.11 nlm "$@"
      '')
    ];
  };
  ...
  basicMemoryPackage = pkgs.symlinkJoin {
    ...
    paths = [
      (pkgs.writeShellScriptBin "basic-memory" ''
        ...
        export UV_PYTHON_DOWNLOADS=never
        exec ${lib.getExe pkgs.uv} tool run --python ${lib.getExe pkgs.python3} --from basic-memory==0.23.2 basic-memory "$@"
      '')
      (pkgs.writeShellScriptBin "basic-memory-mcp" ''
        export BASIC_MEMORY_NO_PROMOS=1
        export BASIC_MEMORY_FORCE_LOCAL=true
        export UV_PYTHON_DOWNLOADS=never
        exec ${lib.getExe pkgs.uv} tool run --python ${lib.getExe pkgs.python3} --from basic-memory==0.23.2 basic-memory mcp "$@"
      '')
    ];
  };
