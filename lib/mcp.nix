  ...
  notebooklmPackage = pkgs.symlinkJoin {
    ...
    paths = [
      (pkgs.writeShellScriptBin "notebooklm-mcp" ''
        ...
        exec ${lib.getExe pkgs.uv} tool run --python ${lib.getExe pkgs.python3} --from notebooklm-mcp-cli==0.7.6 notebooklm-mcp "$@"
      '')
      (pkgs.writeShellScriptBin "nlm" ''
        export UV_PYTHON_DOWNLOADS=never
        exec ${lib.getExe pkgs.uv} tool run --python ${lib.getExe pkgs.python3} --from notebooklm-mcp-cli==0.7.6 nlm "$@"
      '')
    ];
  };
