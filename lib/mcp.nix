  ...
  notebooklmPackage = pkgs.symlinkJoin {
    ...
    paths = [
      (pkgs.writeShellScriptBin "notebooklm-mcp" ''
        ...
        exec ${lib.getExe pkgs.uv} tool run --python ${lib.getExe pkgs.python3} --from notebooklm-mcp-cli==0.12.0 notebooklm-mcp "$@"
      '')
      (pkgs.writeShellScriptBin "nlm" ''
        export UV_PYTHON_DOWNLOADS=never
        exec ${lib.getExe pkgs.uv} tool run --python ${lib.getExe pkgs.python3} --from notebooklm-mcp-cli==0.12.0 nlm "$@"
      '')
    ];
  };
